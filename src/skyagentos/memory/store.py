...
class MemoryStore:
    ...
    def save_mission(self, mission: Mission) -> None:
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "INSERT OR REPLACE INTO missions (id,payload) VALUES (?,?)",
                (mission.id, mission.model_dump_json()),
            )
    ...
    def enqueue(self, run_id: str, payload: dict[str, Any]) -> None:
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "INSERT INTO queue_jobs (run_id, payload, state) VALUES (?,?,'queued')",
    ...
    def get_run_control(self, run_id: str) -> str:
        with sqlite3.connect(self.db_path) as conn:
            row = conn.execute(
                "SELECT status FROM run_controls WHERE run_id=?", (run_id,)
            ).fetchone()
    ...
    def push_episodic(self, namespace: str, content: str) -> None:
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "INSERT INTO episodic_memory (namespace,content) VALUES (?,?)", (namespace, content)
            )
