...
from skyagentos.models.schemas import (
    ErrorType,
    Mission,
    Run,
    RunState,
...
class Orchestrator:
    def __init__(
        ...
        litellm_key: str,
        skyvern_url: str,
        stream_fn: StreamFn = default_stream,
    ):
        ...
        self.desktop = DesktopTool(
            os.getenv("DESKTOP_DAEMON_URL", "http://desktop-daemon:8890"),
            artifact_dir=Path("workspace_artifacts/desktop"),
        )
    ...
    def _execute_run(self, run: Run, mission: Mission) -> dict:
        ...
        if requires_human_review(mission):
            ...
            return {
                ...
                "reason": "domain requires human review",
            }
        ...
        for i in range(1, mission.max_steps + 1):
            ...
            try:
                if runtime == "desktop":
                    result, artifact = self.desktop.execute(
                        run.id,
                        exec_step.id,
                        "operate",
                        {"prompt": mission.objective, "iteration": i},
                    )
                else:
                    payload = {
                ...
                self.store.save_telemetry(
                    TelemetryEvent(
                        ...
                        tags={
                            "passed": str(parsed.passed).lower(),
                            "iteration": str(i),
                            "runtime": runtime,
                        },
                    )
                ...
                self.store.push_semantic(
                    mission.domain, json.dumps(result)[:500], embedding_hint=f"{runtime}-result"
                )
                ...
                if parsed.passed:
                    ...
                    self.stream(
                        "progress",
                        {"run_id": run.id, "state": run.state.value, "step": i, "runtime": runtime},
                    )
                    return outcome
                ...
                self.stream(
                    "progress",
                    {"run_id": run.id, "state": run.state.value, "reason": parsed.reason},
                )
                ...
                self.store.save_telemetry(
                    TelemetryEvent(
                        ...
                        name="step_error",
                        value=1.0,
                        tags={"type": et.value, "runtime": runtime},
                    )
                )
    ...
    def _parse_validation(raw: str) -> ValidationResult:
        ...
        return ValidationResult(
            passed=False, reason="validation parsing failed", next_action="retry"
        )
