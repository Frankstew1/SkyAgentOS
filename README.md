...
This repository now treats **`src/skyagentos` as the canonical orchestrator runtime**.
- Docker services run `python -m skyagentos.api.main ...`
- OpenClaw triggers call `POST /missions`
- Root `main_orchestrator.py` is only a thin module entry shim.
...
- `ollama-pull` preloads `mistral:latest` so `local_reflector` calls do not fail on cold start.
- `desktop-daemon` is included in compose and wired to the orchestrator.
- Skyvern is configured for self-hosted Postgres + LiteLLM `/v1` gateway.

## Quickstart
```bash
cp .env.example .env
PYTHONPATH=src:. pytest -q
PYTHONPATH=src python -m skyagentos.api.main serve
```
